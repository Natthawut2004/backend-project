...
const sql = `
  SELECT
    ...
    (SELECT COUNT(*) FROM enroll WHERE enroll.CourseID = c.CourseID) AS StudentCount,
    (
      SELECT JSON_ARRAYAGG(JSON_OBJECT('SubjectId', tcd.SubjectId, 'SubjectName', s.SubjectName))
      FROM tutorcoursedetails tcd
      JOIN subjects s ON s.SubjectId = tcd.SubjectId
      WHERE tcd.CourseID = c.CourseID
    ) AS Subjects
  FROM courses c
...
router.get('/:id/preview-videos', async (req, res) => {
  try {
    ...
    console.error('Error in GET /courses/:id/preview-videos:', e);
    res.status(500).json({ message: 'Server error' });
  }
});
